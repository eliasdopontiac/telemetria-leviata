...
class FardriverSerial:
    """
    Backend de comunicação Serial robusto.
    Define limites de segurança e comandos de escrita.
    """
    # Constantes estáticas para acesso imediato pela classe
    LIMIT_LINE_CURR = 300
    ...
    def __init__(self):
        # Redundância: garante que os limites existem na instância do objeto
    ...
    # --- NOVOS MÉTODOS REINTEGRADOS PARA CORRIGIR OS ERROS DA UI ---

    def enviar_configuracoes(self, config_dict):
        """Placeholder para envio de parâmetros (EEPROM Write)"""
        if not self.conectado: return False
        print(f"[COMANDO] Gravar Parâmetros: {config_dict}")
        # Lógica de protocolo para escrita seria implementada aqui
        return True

    def iniciar_autolearn(self):
        """Inicia o processo de Auto-Aprendizado via porta serial"""
        if not self.conectado: return False
        print("[COMANDO] Iniciar Auto-Learn...")
        # Lógica de protocolo para autolearn seria implementada aqui
        return True

    def restaurar_fabrica(self):
        """Envia comando de Reset para as definições de fábrica"""
        if not self.conectado: return False
        print("[COMANDO] Restaurar Fábrica!")
        # Lógica de protocolo para factory reset seria implementada aqui
        return True
...
import { X, FolderSimple, Plus, DownloadSimple, Trash, CheckCircle } from '@phosphor-icons/react';
    ...
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: 560 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 12, marginBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><FolderSimple size={18} weight="duotone" color="#00f2fe" /><h3 style={{ fontSize: 15, fontWeight: 700 }}>Sessões de Telemetria</h3></div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)' }}><X size={18} weight="bold" /></button>
        </div>
        ...
        <form onSubmit={criar} style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          ...
          <button type="submit" className="btn btn-accent" disabled={loading || !novoNome.trim()} style={{ fontSize: 11 }}><Plus size={14} weight="bold" />Criar</button>
        </form>
            ...
            return (
              <div key={s.id} style={{ padding: 10, borderRadius: 10, border: `1px solid ${isAtiva ? 'rgba(0,242,254,0.4)' : 'rgba(255,255,255,0.06)'}`, background: isAtiva ? 'rgba(0,242,254,0.06)' : 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                  {!isAtiva && <button className="btn" style={{ fontSize: 10, padding: '3px 8px' }} onClick={() => selecionar(s.id)} disabled={loading}><CheckCircle size={12} weight="duotone" color="#00f2fe" />Ativar</button>}
                  <a className="btn" style={{ fontSize: 10, padding: '3px 8px' }} href={getDownloadCsvUrl(s.id)} download><DownloadSimple size={12} weight="duotone" color="#ffb703" />CSV</a>
                  <button onClick={() => deletar(s.id, s.nome)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', padding: 4, borderRadius: 6 }}><Trash size={14} weight="duotone" /></button>
                </div>
              </div>
            );
          })}

...
import FaultHistoryWidget from '../components/FaultHistoryWidget';
import { Lightning, Gauge, NavigationArrow, Fire, Thermometer, CellSignalFull } from '@phosphor-icons/react';
  ...
  return (
    <>
      ...
      <div className="dashboard-grid">
        {/* COLUNA ESQUERDA: Manômetros + Bateria + Gráfico de Corrente */}
        <div className="dashboard-col">
          <div className="gauge-row">
            <CircularGauge title="Velocidade" value={vel} unit="km/h" min={0} max={30} warningThreshold={22} dangerThreshold={26} color="#00f2fe" icon={NavigationArrow} />
            ...
            <CircularGauge title="Solar" value={potSolar} unit="W" min={0} max={500} color="#ffb703" icon={Lightning} />
          </div>
          <BatteryWidget telemetryData={t} />
          <EnergyChartWidget history={history} />
        </div>

        {/* COLUNA CENTRAL: Mapa Ampliado + Cronômetro Compacto */}
        <div className="dashboard-col">
        ...
        {/* COLUNA DIREITA: Métricas Críticas (Temperaturas, Corrente) + Histórico de Falhas */}
        <div className="dashboard-col" style={{ gap: '12px', display: 'flex', flexDirection: 'column' }}>
          <MetricCard
            title="Temp. Motor" value={tMotor.toFixed(1)} unit="°C"
            icon={Fire} colorScheme="danger" warning={65} danger={85}
          ...
          <MetricCard
            ...
            icon={Lightning} colorScheme="solar"
            subtext="Consumo"
          />
          <MetricCard
            title="Sinal LoRa" value={lora ? `${lora}` : '--'} unit="dBm"
            icon={CellSignalFull} colorScheme="primary"
            progress={Math.max(0, 100 + (lora > 0 ? -lora : lora))} progressMax={100}
            subtext={`Pkts: ${pkts}`}
          />
          <FaultHistoryWidget history={history} currentTelemetry={t} />
        </div>
      </div>
    </>
  );

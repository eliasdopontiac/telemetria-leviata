...
// Handler de dados vindo do Simulador
simulator.onData((pkgObj) => {
  processarEDistribuir(pkgObj, 'simulador');
});
...
app.post('/api/simulador/start', (req, res) => {
  ...
  io.emit('simulador_status', { isRunning: true });
  res.json({ isRunning: true });
});

app.post('/api/simulador/stop', (req, res) => {
  simulator.stop();
  io.emit('simulador_status', { isRunning: false });
  res.json({ isRunning: false });
});

// Injeção de payload direto (para simulação via HTTP/Postman)
app.post('/api/simular-payload', async (req, res) => {
  try {
    await processarEDistribuir(req.body, 'http_simulator');
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
io.on('connection', (socket) => {
  ...
  socket.emit('simulador_status', { isRunning: simulator.isRunning });
